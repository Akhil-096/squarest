...
class LikedProjectCard extends StatefulWidget {
  final Project project;

  const LikedProjectCard({
    Key? key,
    required this.project,
  }) : super(key: key);

  @override
  State<LikedProjectCard> createState() => _LikedProjectCardState();
}

class _LikedProjectCardState extends State<LikedProjectCard> {
  final PageController _controller = PageController(
    initialPage: 0,
  );

  // Future<List<String>> imgUrlList = Future(() => []);
  ...
  Widget build(BuildContext context) {
    ...
    // }
    return Screenshot(
        controller: screenshotController, child: contentWidget(context));
  }
  ...
  Card contentWidget(BuildContext ctx) {
    ...
    // final autoCompleteNotifier =
    //     Provider.of<AutocompleteNotifier>(context);
    // if(autoCompleteNotifier.openShare){
    //   autoCompleteNotifier.openShareAfterLogin(context, screenshotController);
    // }
    return Card(
                            ...
                            child: LikeButton(
                              ...
                              onTap: !userSnapshot.hasData ? (_) async {
                                ...
                                if(!likedProjectsNotifier.isLiked){
                                  likedProjectsNotifier.insertLikes((userProfileNotifier.userProfileModel.id).toInt(), widget.project.id);
  ...
  //     for(int i = 0 ; i < autoCompleteNotifier.projectList.length; i++) {
  //       autoCompleteNotifier.projectList[i].imageUrlList = autoCompleteNotifier.projectList[i].imageUrlList..addAll((await storageAccess
  //           .loadFromBucket(autoCompleteNotifier.projectList[i].applicationno)));
  //     }
  //   } catch (error) {
  //     if (kDebugMode) {
  //       print('$error');
